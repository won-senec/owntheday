...
function toggleTimer() {
  if (!isRunning) {
    // 1. Calculate and save the EXACT time the timer should end
...
function startTimer() {
  if (timer) return;
  timer = setInterval(() => {
    // 2. Instead of just subtracting 1, sync with the real clock
...
// 3. Add this to your existing "visibilitychange" listener
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") {
    const endTime = Number(localStorage.getItem("pomodoroEndTime"));
    if (endTime && isRunning) {
      // Recalculate time remaining based on the gap in time
      timeLeft = Math.max(0, Math.round((endTime - Date.now()) / 1000));
      updateTimer();
    }
    restoreMantraTimer(); // Keep your existing mantra restore
  }
});
